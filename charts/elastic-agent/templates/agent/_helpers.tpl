...
{{- $definedAgentStateVolume := false -}}
  ...
  - name: agent-data
    ...
    hostPath:
      ...
      path: {{ $.Values.agent.fleet.pathPrefix }}/etc/elastic-agent/{{$.Release.Namespace}}/{{$agentName}}-managed/state
      {{- else }}
      {{/* different state hostPath for standalone agents */}}
      path: {{ $.Values.agent.fleet.pathPrefix }}/etc/elastic-agent/{{$.Release.Namespace}}/{{$agentName}}/state
      {{- end }}
